...
const LEVEL_KEYS = {
  faible: { label: "scanner.level.excellent", subtitle: "scanner.level.excellentSub" },
  moyen: { label: "scanner.level.warning", subtitle: "scanner.level.warningSub" },
  eleve: { label: "scanner.level.danger", subtitle: "scanner.level.dangerSub" },
} as const;
...
export default function GlobalScoreCard({ score, level, letterGrade, url, pagesScanned, sitemapFound }: GlobalScoreCardProps) {
  ...
  return (
    <div className={`${styles.bg} ${styles.border} border rounded-2xl p-8 shadow-sm`}>
      <div className="flex flex-col sm:flex-row items-center gap-8">
        {/* Big score circle */}
        <div className="relative w-40 h-40 shrink-0">
          <svg className="w-40 h-40 -rotate-90" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="54" fill="none" strokeWidth="10" className={styles.trackColor} />
            <circle
              cx="60"
              cy="60"
              r="54"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`text-4xl font-bold ${styles.color}`}>{score}</span>
        ...
        <div className="text-center sm:text-left">
          ...
          <h2 className={`text-2xl font-bold ${styles.color} mb-1`}>
            {t(keys.label)}
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            {pagesScanned != null && pagesScanned > 0 && (
              <span className="inline-flex items-center gap-1 text-xs bg-white/60 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 rounded-full px-2.5 py-1 text-gray-600 dark:text-gray-400">
                ...
                {pagesScanned} {pagesScanned > 1 ? t("scanner.pagesAnalyzedPlural") : t("scanner.pagesAnalyzed")}
              </span>
            )}
            {sitemapFound != null && (
              <span className={`inline-flex items-center gap-1 text-xs rounded-full px-2.5 py-1 ${
                ...
                {sitemapFound ? t("scanner.sitemapFound") : t("scanner.noSitemapBadge")}
              </span>
            )}
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
            {t("scanner.scoreDescription")}
          </p>
        </div>
      </div>
    </div>
