...
export default function LegalPagesCard({ legalPages, isEcommerce }: { legalPages: LegalPages; isEcommerce: boolean }) {
  ...
  // CGV are excluded from the count only if not e-commerce AND not detected
  const applicableItems = LEGAL_ITEMS.filter((item) => item.key !== "cgv" || isEcommerce || legalPages.cgv);
  const found = applicableItems.filter((item) => legalPages[item.key]).length;
  const allFound = found === applicableItems.length;

  return (
    <div className={`border rounded-xl p-5 shadow-sm ${allFound ? "bg-green-50 dark:bg-green-950 border-green-300 dark:border-green-700" : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"}`}>
